...
def create_jigsaw(image: Image.Image, m: int, n: int,swap_mode="random"):
    assert swap_mode in ["random","single"], "swap mode must be one of 'random','single'"
...
viewed = False
def process_data(x,draw_grid,use_prompt,use_thinking=False,few_shot=False,use_tool=False,jigsaw_h=2,jigsaw_w=2,swap_mode="random",index_type="1d"):
    global viewed
    image = x["image"]
    ...
    prompt = get_prompt(use_prompt,use_thinking,use_tool,few_shot,jigsaw_h,jigsaw_w,index_type, not viewed)
    if not viewed:
        viewed = True
    msg = [
        {"role":"user","content":[
...
class JigsawR1(BaseDataLoader):
    def load_data(self,dataset_path="jigsaw-r1/coco",
        ...
        self.data = self.data.map(lambda x: process_data(x,draw_grid,use_prompt,use_thinking,few_shot,use_tool,jigsaw_h,jigsaw_w,swap_mode,index_type),batched=False)
